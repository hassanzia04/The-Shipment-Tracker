...
import { documentsApi, type MiscAnalysisItem } from '@/api/documents'
...
interface RowState {
  file: File
  analysis: MiscAnalysisItem | null
  analyzing: boolean
  shipmentId: string | null
  blNumber: string | null
  existingMiscCount: number
...
export function BulkMiscUploadModal({ onClose, onDone }: Props) {
  ...
  const onDrop = useCallback(async (accepted: File[]) => {
    ...
    try {
      const { data } = await documentsApi.analyzeMisc(accepted)
      setRows(prev => {
        const updated = [...prev]
        ...
        data.forEach((item, i) => {
          ...
          if (updated[idx]) {
            updated[idx] = {
              ...
              existingMiscCount: item.existing_misc_count,
            }
          }
        })
        return updated
  ...
  async function handleUpload() {
    ...
    await Promise.all(
      rows.map(async (row, idx) => {
        if (!row.shipmentId || row.uploadStatus !== 'idle') return
        setRows(prev => {
          const updated = [...prev]
        ...
        try {
          await documentsApi.upload({ shipment_id: row.shipmentId, doc_type: 'MISCELLANEOUS', file: row.file })
          setRows(prev => {
            const updated = [...prev]
  ...
  function setManualMatch(idx: number, shipmentId: string, blNumber: string) {
    setRows(prev => {
      const updated = [...prev]
      updated[idx] = { ...updated[idx], shipmentId, blNumber }
      return updated
    })
  }
  ...
  function unmatchRow(idx: number) {
    setRows(prev => {
      const updated = [...prev]
      updated[idx] = { ...updated[idx], shipmentId: null, blNumber: null, existingMiscCount: 0 }
      return updated
    })
  }

  const readyCount = rows.filter(r => r.shipmentId && r.uploadStatus === 'idle').length
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b dark:border-gray-700">
          <h2 className="text-base font-semibold text-gray-900 dark:text-white">Bulk Misc Document Upload</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          >
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300">
              {isDragActive ? 'Drop documents here' : 'Drop documents here, or click to browse'}
            </p>
            <p className="text-xs text-gray-400 mt-1">PDF, PNG, JPG · multiple files supported · include the BL number in the file name for auto-matching</p>
          </div>
        ...
        row.shipmentId ? (
          <div className="flex flex-col gap-1.5 ml-7">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {row.existingMiscCount > 0 && (
                <span className="text-xs text-gray-400">
                  {row.existingMiscCount} misc doc{row.existingMiscCount !== 1 ? 's' : ''} already on this shipment
                </span>
              )}
              <button
          ...
          <div className="ml-7">
            <p className="text-xs text-amber-600 dark:text-amber-400 mb-1">
              {row.analysis?.detected_bl
                ...
                : 'Could not detect a BL number in the file name'}
            </p>
            {!searching && (
              <button
                onClick={startSearching}
